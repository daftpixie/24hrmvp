 ...
 */
function formatUserProfile(user: any, stats: any) {
  return {
    id: user.id,
    fid: user.fid,
    ...
    wallets: user.cryptoWallets?.map((w: any) => ({
      id: w.id,
      address: w.address,
...
router.get('/:userId/ideas', asyncHandler(async (req: Request, res: Response) => {
  const { userId } = req.params;
...
router.get('/:userId/activity', asyncHandler(async (req: Request, res: Response) => {
  const { userId } = req.params;
...
router.get('/', asyncHandler(async (req: Request, res: Response) => {
  ...
  const [users, total] = await Promise.all([
    prisma.user.findMany({
      where: whereClause,
      orderBy: orderByMap[sortBy as string] || orderByMap.points,
      take: parseInt(limit as string),
      skip: parseInt(offset as string),
      select: {
        id: true,
