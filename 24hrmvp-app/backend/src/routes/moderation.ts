...
import { ModerationDecisionDTO, ModerationQueueParams } from '../types/grid';
  ...
  ...requireAdmin(async (req: Request, res: Response): Promise<void> => {
    try {
      const params: ModerationQueueParams = {
        ...
        entityType: req.query.entityType as ModerationEntityType | undefined,
        page: parseInt(req.query.page as string) || 1,
        limit: Math.min(parseInt(req.query.limit as string) || 20, 50),
      };

      const result = await moderationService.getQueue(params);
...
router.get(
  ...
  ...requireAdmin(async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const item = await moderationService.getQueueItem(id);
...
router.post(
  ...
  ...requireAdmin(async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Validate action
      const validActions = Object.values(ModerationStatus);
      ...
      const item = await moderationService.review(id, req.user!.id, decision);
...
router.post(
  ...
  ...requireAuth(async (req, res) => {
    try {
      ...
      const validTypes = Object.values(ModerationEntityType);
