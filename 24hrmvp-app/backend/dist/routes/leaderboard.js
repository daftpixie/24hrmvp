...
router.get('/', gridAuth_1.optionalAuth, async (req, res) => {
    try {
        ...
        if (cached) {
            ...
            const user = (0, gridAuth_1.getAuthUser)(req);
            if (user) {
        ...
        // Cache for 1 minute
        await redis_1.cache.set(cacheKey, response, 60);
        res.json({
            success: true,
            ...response,
        });
 ...
 */
router.get('/me', gridAuth_1.optionalAuth, async (req, res) => {
    try {
        const user = (0, gridAuth_1.getAuthUser)(req);
        if (!user) {
            res.status(401).json({
        ...
        const metrics = ['points', 'submissions', 'votes', 'forum_score', 'achievements'];
        const ranks = {};
...
// HELPER FUNCTIONS
// ============================================
async function getLeaderboardEntries(metric, timeframe, limit) {
    // Build date filter
    ...
    switch (metric) {
        ...
        case 'forum_score':
            ...
            const forumUsers = await client_1.prisma.user.findMany({
                where: dateFilter ? { createdAt: { gte: dateFilter } } : undefined,
                select: {
                    id: true,
                    fid: true,
...
async function getUserRank(userId, metric) {
    ...
    let rank;
    if (metric === 'points') {
        rank = await client_1.prisma.user.count({ where: { points: { gt: score } } }) + 1;
    }
        ...
        const higherScores = allUsers.filter((u) => u.forumPosts.reduce((sum, p) => sum + p.upvotes, 0) > score).length;
        rank = higherScores + 1;
    }
    else {
