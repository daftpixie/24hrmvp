...
// VALIDATION SCHEMAS
// ============================================
const PurchaseVotesSchema = zod_1.z.object({
    packageId: zod_1.z.string(), // e.g., "single", "pack_5", "pack_20", "pack_100"
    paymentMethod: zod_1.z.enum(['stripe', 'coinbase', 'points']),
    returnUrl: zod_1.z.string().url().optional(), // For Stripe redirects
});
// ============================================
// HELPER FUNCTIONS
// ============================================
...
async function ensureVotePricing() {
    ...
    for (const pkg of packages) {
        await index_1.prisma.votePricing.upsert({
 ...
 */
async function getAvailableVoteCredits(userId) {
    const credits = await index_1.prisma.voteCredit.findMany({
        where: {
            userId,
            isExpired: false,
            remaining: { gt: 0 },
 ...
 */
async function awardDailyVoteCredits(userId) {
...
router.get('/pricing', async (req, res) => {
    try {
        ...
        const pricing = await index_1.prisma.votePricing.findMany({
            where: { isActive: true },
 ...
 */
router.get('/credits', auth_1.verifyFarcasterAuth, async (req, res) => {
    try {
        if (!req.user) {
 ...
 */
router.post('/stripe', auth_1.verifyFarcasterAuth, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({
                error: 'Unauthorized',
        ...
        // Get pricing package
        const package_ = await index_1.prisma.votePricing.findUnique({
        ...
        const purchase = await index_1.prisma.votePurchase.create({
            data: {
                userId: user.id,
 ...
 */
router.post('/coinbase', auth_1.verifyFarcasterAuth, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({
                error: 'Unauthorized',
 ...
 */
router.post('/points', auth_1.verifyFarcasterAuth, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({
                error: 'Unauthorized',
...
//# sourceMappingURL=vote-purchase.js.map