"use strict";
// ============================================
// 24HRMVP - REDIS SERVICE (Optional)
// File: backend/src/services/redis.ts
// Falls back to memory when Redis unavailable
// Phase 3B: Added isRedisConnected() export
// ============================================
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.subscriber = exports.publisher = exports.redis = exports.gridCache = exports.pubsub = exports.rateLimiter = exports.leaderboard = exports.cache = void 0;
exports.isRedisConnected = isRedisConnected;
exports.checkRedisHealth = checkRedisHealth;
const ioredis_1 = __importDefault(require("ioredis"));
// ============================================
// CONFIGURATION
// ============================================
const REDIS_ENABLED = !!process.env.REDIS_URL;
// ============================================
// REDIS CLIENTS (only if configured)
// ============================================
let redis = null;
exports.redis = redis;
let publisher = null;
exports.publisher = publisher;
let subscriber = null;
exports.subscriber = subscriber;
let redisConnected = false;
if (REDIS_ENABLED) {
    try {
        exports.redis = redis = new ioredis_1.default(process.env.REDIS_URL, {
            maxRetriesPerRequest: null,
            enableReadyCheck: true,
            lazyConnect: true,
            retryStrategy: (times) => {
                if (times > 3) {
                    console.log('  ℹ Redis unavailable, using memory fallback');
                    return null;
                }
                return Math.min(times * 100, 1000);
            },
        });
        exports.publisher = publisher = redis.duplicate();
        exports.subscriber = subscriber = redis.duplicate();
        redis.on('connect', () => {
            redisConnected = true;
            console.log('✓ Redis connected');
        });
        redis.on('error', () => {
            redisConnected = false;
        });
        redis.on('close', () => {
            redisConnected = false;
        });
        // Attempt connection
        redis.connect().catch(() => {
            console.log('  ℹ Redis not available, using memory fallback');
        });
    }
    catch (err) {
        console.log('  ℹ Redis initialization failed, using memory fallback');
    }
}
else {
    console.log('  ℹ Redis not configured, using memory fallback');
}
// ============================================
// PHASE 3B: Connection Status Export
// ============================================
function isRedisConnected() {
    return redisConnected;
}
const memoryCache = new Map();
const memorySortedSets = new Map();
// Clean expired entries every minute
setInterval(() => {
    const now = Date.now();
    for (const [key, entry] of memoryCache.entries()) {
        if (entry.expiry && entry.expiry < now) {
            memoryCache.delete(key);
        }
    }
}, 60000);
// ============================================
// CACHE OPERATIONS
// ============================================
exports.cache = {
    async get(key) {
        if (redisConnected && redis) {
            try {
                const value = await redis.get(key);
                if (value) {
                    try {
                        return JSON.parse(value);
                    }
                    catch {
                        return value;
                    }
                }
                return null;
            }
            catch {
                // Fall through to memory
            }
        }
        const entry = memoryCache.get(key);
        if (!entry)
            return null;
        if (entry.expiry && entry.expiry < Date.now()) {
            memoryCache.delete(key);
            return null;
        }
        return entry.value;
    },
    async set(key, value, ttlSeconds) {
        const stringValue = typeof value === 'string' ? value : JSON.stringify(value);
        if (redisConnected && redis) {
            try {
                if (ttlSeconds) {
                    await redis.setex(key, ttlSeconds, stringValue);
                }
                else {
                    await redis.set(key, stringValue);
                }
                return;
            }
            catch {
                // Fall through to memory
            }
        }
        memoryCache.set(key, {
            value,
            expiry: ttlSeconds ? Date.now() + ttlSeconds * 1000 : null,
        });
    },
    async del(key) {
        if (redisConnected && redis) {
            try {
                await redis.del(key);
            }
            catch {
                // Continue to memory
            }
        }
        memoryCache.delete(key);
    },
    async exists(key) {
        if (redisConnected && redis) {
            try {
                return (await redis.exists(key)) === 1;
            }
            catch {
                // Fall through
            }
        }
        const entry = memoryCache.get(key);
        if (!entry)
            return false;
        if (entry.expiry && entry.expiry < Date.now()) {
            memoryCache.delete(key);
            return false;
        }
        return true;
    },
};
// ============================================
// LEADERBOARD OPERATIONS (Sorted Sets)
// ============================================
exports.leaderboard = {
    async addScore(key, oderId, score) {
        if (redisConnected && redis) {
            try {
                await redis.zadd(key, score, oderId);
                return;
            }
            catch {
                // Fall through
            }
        }
        let set = memorySortedSets.get(key);
        if (!set) {
            set = new Map();
            memorySortedSets.set(key, set);
        }
        set.set(oderId, (set.get(oderId) || 0) + score);
    },
    async getTopN(key, n) {
        if (redisConnected && redis) {
            try {
                const results = await redis.zrevrange(key, 0, n - 1, 'WITHSCORES');
                const entries = [];
                for (let i = 0; i < results.length; i += 2) {
                    entries.push({ oderId: results[i], score: parseFloat(results[i + 1]) });
                }
                return entries;
            }
            catch {
                // Fall through
            }
        }
        const set = memorySortedSets.get(key);
        if (!set)
            return [];
        return Array.from(set.entries())
            .sort((a, b) => b[1] - a[1])
            .slice(0, n)
            .map(([oderId, score]) => ({ oderId, score }));
    },
    async getRank(key, oderId) {
        if (redisConnected && redis) {
            try {
                const rank = await redis.zrevrank(key, oderId);
                return rank !== null ? rank + 1 : null;
            }
            catch {
                // Fall through
            }
        }
        const set = memorySortedSets.get(key);
        if (!set)
            return null;
        const sorted = Array.from(set.entries()).sort((a, b) => b[1] - a[1]);
        const index = sorted.findIndex(([id]) => id === oderId);
        return index >= 0 ? index + 1 : null;
    },
    async getScore(key, oderId) {
        if (redisConnected && redis) {
            try {
                const score = await redis.zscore(key, oderId);
                return score !== null ? parseFloat(score) : null;
            }
            catch {
                // Fall through
            }
        }
        const set = memorySortedSets.get(key);
        return set?.get(oderId) ?? null;
    },
};
// ============================================
// RATE LIMITER
// ============================================
const rateLimitMemory = new Map();
exports.rateLimiter = {
    async checkLimit(key, limit, windowSeconds) {
        if (redisConnected && redis) {
            try {
                const current = await redis.incr(key);
                if (current === 1) {
                    await redis.expire(key, windowSeconds);
                }
                return {
                    allowed: current <= limit,
                    remaining: Math.max(0, limit - current),
                };
            }
            catch {
                // Fall through
            }
        }
        const now = Date.now();
        let entry = rateLimitMemory.get(key);
        if (!entry || entry.resetAt < now) {
            entry = { count: 0, resetAt: now + windowSeconds * 1000 };
            rateLimitMemory.set(key, entry);
        }
        entry.count++;
        return {
            allowed: entry.count <= limit,
            remaining: Math.max(0, limit - entry.count),
        };
    },
};
// ============================================
// PUB/SUB
// ============================================
exports.pubsub = {
    async publish(channel, message) {
        if (redisConnected && publisher) {
            try {
                await publisher.publish(channel, message);
            }
            catch {
                // Silent fail - no pub/sub without Redis
            }
        }
    },
    subscribe(channel, callback) {
        if (redisConnected && subscriber) {
            try {
                subscriber.subscribe(channel);
                subscriber.on('message', (ch, msg) => {
                    if (ch === channel)
                        callback(msg);
                });
            }
            catch {
                // Silent fail
            }
        }
    },
};
// ============================================
// GRID CACHE (Forum/Social specific)
// ============================================
exports.gridCache = {
    // TTL constants
    ttl: {
        feed: 60,
        post: 300,
        trending: 120,
    },
    // Key generators
    keys: {
        forumFeed: (params) => `grid:forum:feed:${params}`,
        forumPost: (slug) => `grid:forum:post:${slug}`,
        socialFeed: (platform, channelId) => `grid:social:feed:${platform || "all"}:${channelId || "none"}`,
        trending: (channelId) => `grid:trending:${channelId}`,
    },
    async getForumFeed(params) {
        return exports.cache.get(`grid:forum:feed:${params}`);
    },
    async setForumFeed(params, data, ttl = 60) {
        await exports.cache.set(`grid:forum:feed:${params}`, data, ttl);
    },
    async invalidateForumCache() {
        for (const key of memoryCache.keys()) {
            if (key.startsWith('grid:forum:')) {
                memoryCache.delete(key);
            }
        }
    },
    async invalidatePost(slug) {
        await exports.cache.del(`grid:forum:post:${slug}`);
        await this.invalidateForumCache();
    },
    async invalidateSocialFeed(platform) {
        const prefix = platform ? `grid:social:feed:${platform}` : 'grid:social:';
        for (const key of memoryCache.keys()) {
            if (key.startsWith(prefix)) {
                memoryCache.delete(key);
            }
        }
    },
};
// ============================================
// HEALTH CHECK
// ============================================
async function checkRedisHealth() {
    if (!REDIS_ENABLED) {
        return { status: 'disabled', connected: false };
    }
    if (!redisConnected || !redis) {
        return { status: 'disconnected', connected: false };
    }
    try {
        const start = Date.now();
        await redis.ping();
        return { status: 'connected', connected: true, latency: Date.now() - start };
    }
    catch {
        return { status: 'disconnected', connected: false };
    }
}
exports.default = {
    cache: exports.cache,
    leaderboard: exports.leaderboard,
    rateLimiter: exports.rateLimiter,
    pubsub: exports.pubsub,
    gridCache: exports.gridCache,
    checkRedisHealth,
};
//# sourceMappingURL=redis.js.map