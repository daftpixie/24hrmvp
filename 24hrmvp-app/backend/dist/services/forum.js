"use strict";
// ============================================
// 24HRMVP - THE GRID FORUM SERVICE
// File: backend/src/services/forum.ts
// Forum CRUD operations with ranking
// ============================================
Object.defineProperty(exports, "__esModule", { value: true });
exports.forumService = exports.ForumService = void 0;
const client_1 = require("../db/client");
const client_2 = require("@prisma/client");
const ranking_1 = require("./ranking");
const redis_1 = require("./redis");
// Slug generation utility
function generateSlug(title, id) {
    if (title) {
        const baseSlug = title
            .toLowerCase()
            .replace(/[^a-z0-9\s-]/g, '')
            .replace(/\s+/g, '-')
            .substring(0, 50);
        return `${baseSlug}-${id.substring(0, 8)}`;
    }
    return id;
}
class ForumService {
    rankingService;
    constructor() {
        this.rankingService = new ranking_1.RankingService();
    }
    /**
     * Create a new forum post
     */
    async createPost(userId, data) {
        const id = crypto.randomUUID().replace(/-/g, '').substring(0, 25);
        const slug = generateSlug(data.title, id);
        // Calculate depth if it's a reply
        let depth = 0;
        if (data.parentId) {
            const parent = await client_1.prisma.forumPost.findUnique({
                where: { id: data.parentId },
                select: { depth: true },
            });
            if (parent) {
                depth = parent.depth + 1;
            }
        }
        const post = await client_1.prisma.forumPost.create({
            data: {
                id,
                slug,
                title: data.title,
                content: data.content,
                type: data.type || client_2.PostType.DISCUSSION, // UPPERCASE enum
                authorId: userId,
                parentId: data.parentId,
                ideaId: data.ideaId,
                depth,
                tags: data.tags || [],
                moderationStatus: client_2.ModerationStatus.APPROVED, // UPPERCASE enum
            },
            include: {
                author: {
                    select: {
                        id: true,
                        fid: true,
                        username: true,
                        displayName: true,
                        pfpUrl: true,
                    },
                },
                _count: {
                    select: {
                        replies: true,
                        votes: true,
                        bookmarks: true,
                    },
                },
            },
        });
        // Update parent reply count if this is a reply
        if (data.parentId) {
            await client_1.prisma.forumPost.update({
                where: { id: data.parentId },
                data: {
                    replyCount: { increment: 1 },
                    lastReplyAt: new Date(),
                },
            });
        }
        // Invalidate feed caches
        await redis_1.gridCache.invalidatePost(slug);
        return this.mapToDTO(post);
    }
    /**
     * Get a single post by slug
     */
    async getPost(slug, userId) {
        // Check cache first
        const cacheKey = redis_1.gridCache.keys.forumPost(slug);
        const cached = await redis_1.cache.get(cacheKey);
        if (cached && !userId) {
            return cached;
        }
        const post = await client_1.prisma.forumPost.findUnique({
            where: {
                slug,
                moderationStatus: client_2.ModerationStatus.APPROVED, // UPPERCASE
            },
            include: {
                author: {
                    select: {
                        id: true,
                        fid: true,
                        username: true,
                        displayName: true,
                        pfpUrl: true,
                    },
                },
                _count: {
                    select: {
                        replies: true,
                        votes: true,
                        bookmarks: true,
                    },
                },
            },
        });
        if (!post)
            return null;
        // Increment view count
        await client_1.prisma.forumPost.update({
            where: { id: post.id },
            data: { viewCount: { increment: 1 } },
        });
        const dto = this.mapToDTO(post);
        // Get user-specific data if authenticated
        if (userId) {
            const [userVote, bookmark] = await Promise.all([
                client_1.prisma.forumPostVote.findUnique({
                    where: { userId_postId: { userId, postId: post.id } },
                    select: { value: true },
                }),
                client_1.prisma.forumBookmark.findUnique({
                    where: { userId_postId: { userId, postId: post.id } },
                }),
            ]);
            dto.userVote = userVote?.value ?? null;
            dto.isBookmarked = !!bookmark;
        }
        // Cache without user-specific data
        if (!userId) {
            await redis_1.cache.set(cacheKey, dto, redis_1.gridCache.ttl.post);
        }
        return dto;
    }
    /**
     * Update a forum post
     */
    async updatePost(slug, userId, data) {
        const post = await client_1.prisma.forumPost.findUnique({
            where: { slug },
            select: { id: true, authorId: true },
        });
        if (!post) {
            throw new Error('Post not found');
        }
        if (post.authorId !== userId) {
            throw new Error('Not authorized to edit this post');
        }
        const updated = await client_1.prisma.forumPost.update({
            where: { id: post.id },
            data: {
                title: data.title,
                content: data.content,
                tags: data.tags,
                isPinned: data.isPinned,
                isLocked: data.isLocked,
            },
            include: {
                author: {
                    select: {
                        id: true,
                        fid: true,
                        username: true,
                        displayName: true,
                        pfpUrl: true,
                    },
                },
                _count: {
                    select: {
                        replies: true,
                        votes: true,
                        bookmarks: true,
                    },
                },
            },
        });
        await redis_1.gridCache.invalidatePost(slug);
        return this.mapToDTO(updated);
    }
    /**
     * Delete a forum post (soft delete via moderation status)
     */
    async deletePost(slug, userId) {
        const post = await client_1.prisma.forumPost.findUnique({
            where: { slug },
            select: { id: true, authorId: true, parentId: true },
        });
        if (!post) {
            throw new Error('Post not found');
        }
        if (post.authorId !== userId) {
            throw new Error('Not authorized to delete this post');
        }
        // Soft delete by setting moderation status to REJECTED
        await client_1.prisma.forumPost.update({
            where: { id: post.id },
            data: { moderationStatus: client_2.ModerationStatus.REJECTED }, // UPPERCASE
        });
        // Update parent reply count if this was a reply
        if (post.parentId) {
            await client_1.prisma.forumPost.update({
                where: { id: post.parentId },
                data: { replyCount: { decrement: 1 } },
            });
        }
        await redis_1.gridCache.invalidatePost(slug);
    }
    /**
     * Get forum feed with sorting and filtering
     */
    async getFeed(params, userId) {
        const { sort = 'hot', type, timeframe = 'week', page = 1, limit = 20, authorId, ideaId, tags, } = params;
        // Build where clause
        const where = {
            parentId: null, // Top-level posts only
            moderationStatus: client_2.ModerationStatus.APPROVED, // UPPERCASE
        };
        if (type) {
            where.type = type;
        }
        if (authorId) {
            where.authorId = authorId;
        }
        if (ideaId) {
            where.ideaId = ideaId;
        }
        if (tags && tags.length > 0) {
            where.tags = { hasSome: tags };
        }
        // Apply timeframe filter
        if (timeframe !== 'all') {
            const now = new Date();
            const timeframeMap = {
                hour: 60 * 60 * 1000,
                day: 24 * 60 * 60 * 1000,
                week: 7 * 24 * 60 * 60 * 1000,
                month: 30 * 24 * 60 * 60 * 1000,
                year: 365 * 24 * 60 * 60 * 1000,
            };
            where.createdAt = {
                gte: new Date(now.getTime() - timeframeMap[timeframe]),
            };
        }
        // Build order by based on sort type
        const orderBy = this.getSortOrder(sort);
        // Get total count
        const total = await client_1.prisma.forumPost.count({ where });
        // Get posts
        const posts = await client_1.prisma.forumPost.findMany({
            where,
            orderBy,
            skip: (page - 1) * limit,
            take: limit,
            include: {
                author: {
                    select: {
                        id: true,
                        fid: true,
                        username: true,
                        displayName: true,
                        pfpUrl: true,
                    },
                },
                _count: {
                    select: {
                        replies: true,
                        votes: true,
                        bookmarks: true,
                    },
                },
            },
        });
        // Map to DTOs with user-specific data
        const postDTOs = await Promise.all(posts.map(async (post) => {
            const dto = this.mapToDTO(post);
            if (userId) {
                const [userVote, bookmark] = await Promise.all([
                    client_1.prisma.forumPostVote.findUnique({
                        where: { userId_postId: { userId, postId: post.id } },
                        select: { value: true },
                    }),
                    client_1.prisma.forumBookmark.findUnique({
                        where: { userId_postId: { userId, postId: post.id } },
                    }),
                ]);
                dto.userVote = userVote?.value ?? null;
                dto.isBookmarked = !!bookmark;
            }
            return dto;
        }));
        return {
            posts: postDTOs,
            pagination: {
                page,
                limit,
                total,
                pages: Math.ceil(total / limit),
                hasMore: page * limit < total,
            },
        };
    }
    /**
     * Get thread (post with replies)
     */
    async getThread(slug, params, userId) {
        const { page = 1, limit = 20 } = params;
        const post = await this.getPost(slug, userId);
        if (!post) {
            throw new Error('Post not found');
        }
        // Get replies
        const [replies, total] = await Promise.all([
            client_1.prisma.forumPost.findMany({
                where: {
                    parentId: post.id,
                    moderationStatus: client_2.ModerationStatus.APPROVED, // UPPERCASE
                },
                orderBy: [
                    { isPinned: 'desc' },
                    { wilsonScore: 'desc' },
                ],
                skip: (page - 1) * limit,
                take: limit,
                include: {
                    author: {
                        select: {
                            id: true,
                            fid: true,
                            username: true,
                            displayName: true,
                            pfpUrl: true,
                        },
                    },
                    _count: {
                        select: {
                            replies: true,
                            votes: true,
                            bookmarks: true,
                        },
                    },
                },
            }),
            client_1.prisma.forumPost.count({
                where: {
                    parentId: post.id,
                    moderationStatus: client_2.ModerationStatus.APPROVED, // UPPERCASE
                },
            }),
        ]);
        const replyDTOs = await Promise.all(replies.map(async (reply) => {
            const dto = this.mapToDTO(reply);
            if (userId) {
                const userVote = await client_1.prisma.forumPostVote.findUnique({
                    where: { userId_postId: { userId, postId: reply.id } },
                    select: { value: true },
                });
                dto.userVote = userVote?.value ?? null;
            }
            return dto;
        }));
        return {
            post,
            replies: replyDTOs,
            pagination: {
                page,
                limit,
                total,
                hasMore: page * limit < total,
            },
        };
    }
    /**
     * Vote on a post
     */
    async vote(postId, userId, value) {
        const existingVote = await client_1.prisma.forumPostVote.findUnique({
            where: { userId_postId: { userId, postId } },
        });
        const scoreDelta = existingVote ? value - existingVote.value : value;
        await client_1.prisma.$transaction(async (tx) => {
            // Upsert vote
            await tx.forumPostVote.upsert({
                where: { userId_postId: { userId, postId } },
                update: { value },
                create: { userId, postId, value },
            });
            // Update post scores
            const updateData = {
                score: { increment: scoreDelta },
            };
            if (value === 1 && (!existingVote || existingVote.value !== 1)) {
                updateData.upvotes = { increment: 1 };
                if (existingVote?.value === -1) {
                    updateData.downvotes = { decrement: 1 };
                }
            }
            else if (value === -1 && (!existingVote || existingVote.value !== -1)) {
                updateData.downvotes = { increment: 1 };
                if (existingVote?.value === 1) {
                    updateData.upvotes = { decrement: 1 };
                }
            }
            await tx.forumPost.update({
                where: { id: postId },
                data: updateData,
            });
        });
        // Update rankings
        await this.rankingService.updatePostRanking(postId);
        const post = await client_1.prisma.forumPost.findUnique({
            where: { id: postId },
            select: { score: true, slug: true },
        });
        if (post?.slug) {
            await redis_1.gridCache.invalidatePost(post.slug);
        }
        return {
            score: post?.score ?? 0,
            userVote: value,
        };
    }
    /**
     * Remove vote from a post
     */
    async removeVote(postId, userId) {
        const existingVote = await client_1.prisma.forumPostVote.findUnique({
            where: { userId_postId: { userId, postId } },
        });
        if (!existingVote) {
            const post = await client_1.prisma.forumPost.findUnique({
                where: { id: postId },
                select: { score: true },
            });
            return { score: post?.score ?? 0 };
        }
        await client_1.prisma.$transaction(async (tx) => {
            await tx.forumPostVote.delete({
                where: { userId_postId: { userId, postId } },
            });
            const updateData = {
                score: { decrement: existingVote.value },
            };
            if (existingVote.value === 1) {
                updateData.upvotes = { decrement: 1 };
            }
            else {
                updateData.downvotes = { decrement: 1 };
            }
            await tx.forumPost.update({
                where: { id: postId },
                data: updateData,
            });
        });
        await this.rankingService.updatePostRanking(postId);
        const post = await client_1.prisma.forumPost.findUnique({
            where: { id: postId },
            select: { score: true, slug: true },
        });
        if (post?.slug) {
            await redis_1.gridCache.invalidatePost(post.slug);
        }
        return { score: post?.score ?? 0 };
    }
    /**
     * Bookmark a post
     */
    async bookmark(postId, userId) {
        await client_1.prisma.forumBookmark.create({
            data: { userId, postId },
        });
    }
    /**
     * Remove bookmark
     */
    async removeBookmark(postId, userId) {
        await client_1.prisma.forumBookmark.delete({
            where: { userId_postId: { userId, postId } },
        });
    }
    /**
     * Get user's bookmarked posts
     */
    async getUserBookmarks(userId, params) {
        const { page = 1, limit = 20 } = params;
        const [bookmarks, total] = await Promise.all([
            client_1.prisma.forumBookmark.findMany({
                where: { userId },
                orderBy: { createdAt: 'desc' },
                skip: (page - 1) * limit,
                take: limit,
                include: {
                    post: {
                        include: {
                            author: {
                                select: {
                                    id: true,
                                    fid: true,
                                    username: true,
                                    displayName: true,
                                    pfpUrl: true,
                                },
                            },
                            _count: {
                                select: {
                                    replies: true,
                                    votes: true,
                                    bookmarks: true,
                                },
                            },
                        },
                    },
                },
            }),
            client_1.prisma.forumBookmark.count({ where: { userId } }),
        ]);
        const posts = bookmarks
            .filter((b) => b.post.moderationStatus === client_2.ModerationStatus.APPROVED) // UPPERCASE
            .map((b) => ({
            ...this.mapToDTO(b.post),
            isBookmarked: true,
        }));
        return {
            posts,
            pagination: {
                page,
                limit,
                total,
                pages: Math.ceil(total / limit),
                hasMore: page * limit < total,
            },
        };
    }
    /**
     * Get sort order for queries
     */
    getSortOrder(sort) {
        switch (sort) {
            case 'hot':
                return [{ isPinned: 'desc' }, { hotScore: 'desc' }];
            case 'new':
                return [{ isPinned: 'desc' }, { createdAt: 'desc' }];
            case 'top':
                return [{ isPinned: 'desc' }, { wilsonScore: 'desc' }];
            case 'controversial':
                // Posts with high engagement but close to 50/50 split
                return [{ isPinned: 'desc' }, { replyCount: 'desc' }, { score: 'asc' }];
            default:
                return [{ isPinned: 'desc' }, { hotScore: 'desc' }];
        }
    }
    /**
     * Map Prisma model to DTO
     */
    mapToDTO(post) {
        return {
            ...post,
            author: {
                id: post.author.id,
                fid: post.author.fid,
                username: post.author.username,
                displayName: post.author.displayName,
                pfpUrl: post.author.pfpUrl,
            },
            _count: post._count,
            userVote: null,
            isBookmarked: false,
        };
    }
}
exports.ForumService = ForumService;
// Export singleton instance
exports.forumService = new ForumService();
exports.default = exports.forumService;
//# sourceMappingURL=forum.js.map