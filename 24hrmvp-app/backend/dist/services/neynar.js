...
exports.neynarService = exports.NeynarService = void 0;
...
class NeynarService {
    ...
    async getChannelFeed(channelId, cursor, limit = 25) {
        ...
        if (cached)
            return cached;
        const response = await this.request('/farcaster/feed/channel', {
        ...
        // Cache for 2 minutes
        await redis_1.cache.set(cacheKey, result, 120);
        return result;
    }
    /**
     ...
     * @param limit Number of casts to return
     */
    async getUserCasts(fid, cursor, limit = 25) {
        const cacheKey = `neynar:user:${fid}:${cursor || 'first'}`;
        const cached = await redis_1.cache.get(cacheKey);
        if (cached)
            return cached;
        const response = await this.request('/farcaster/feed/user/casts', {
            fid: fid.toString(),
            limit: limit.toString(),
            ...(cursor && { cursor }),
        });
    ...
    async getCast(hash) {
        ...
        try {
            ...
            await redis_1.cache.set(cacheKey, cast, 300); // Cache for 5 minutes
            return cast;
        }
        catch (error) {
    ...
    async searchCasts(query, limit = 10) {
        ...
        try {
            const response = await this.request('/farcaster/cast/search', {
                q: query,
                limit: limit.toString(),
            });
    ...
    async getTrendingChannels(limit = 10) {
        ...
        if (cached)
            return cached;
        try {
            const response = await this.request('/farcaster/channel/trending', { limit: limit.toString() });
            const channels = response.channels.map((ch) => ({
                id: ch.id,
                name: ch.name,
                follower_count: ch.follower_count || 0,
            }));
            await redis_1.cache.set(cacheKey, channels, 600); // Cache for 10 minutes
            return channels;
        }
        catch (error) {
    ...
    async getChannel(channelId) {
        ...
        if (cached)
            return cached;
        try {
            const response = await this.request('/farcaster/channel', { id: channelId });
            const channel = {
            ...
            await redis_1.cache.set(cacheKey, channel, 3600); // Cache for 1 hour
            return channel;
        }
        catch (error) {
    ...
    async getUser(fid) {
        ...
        if (cached)
            return cached;
        try {
            const response = await this.request('/farcaster/user/bulk', { fids: fid.toString() });
            ...
            await redis_1.cache.set(cacheKey, result, 3600); // Cache for 1 hour
            return result;
        }
        catch (error) {
    ...
    normalizeCast(cast) {
        return {
            ...
            mentioned_profiles: cast.mentioned_profiles?.map((p) => ({
                fid: p.fid,
                username: p.username,
...
exports.NeynarService = NeynarService;
// Export singleton instance
exports.neynarService = new NeynarService();
exports.default = exports.neynarService;
